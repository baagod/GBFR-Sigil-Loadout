...
启动那一次写与运行中的热应用**不是两套机制**：它们各走各的入口，在 `Publish` 处汇合成同一次"重新注册 + 原地写内存"。
...
sequenceDiagram
    ...
    Note over Tick,FE: 路径一 启动那一次写
    Tick->>FE: Mod.Start 调 Bootstrap
    FE->>File: 先取 mtime 版本号
    FE->>DM: 读归档原表
    FE->>FE: BuildEditedTable 打行
    FE->>FE: TryApply 带上刚建好的表
    Note over FE: 造表期间文件又变了就不写 不推进版本

    Note over Tool,File: 路径二 运行中的热应用
    Tool->>File: 编辑停下 500ms 后原子写
    Tick->>FE: Tick 每 250ms
    FE->>File: mtime 门 Pending
    FE->>DM: BuildCurrentTable 按磁盘列表重建
    FE->>FE: TryApply 这一版

    Note over FE,Game: 两条路径在此汇合 唯一那条 Publish
    ...
    alt 写成功
        ...
        NC-->>FE: 负数拒绝码
        FE->>FE: 留候选表 不推进版本 同版本 5s 后重试
    end
```

上图：启动那次写（`Bootstrap`）先取版本号再读内容，运行中的热应用由 mtime 门放行；两者都落到 `TryApply` → `Apply` → `Publish`。`bootTable` 参数就是"两条路径是同一个操作"的体现——启动时已经建好的表直接交过去，让 `Tick` 再读一遍配置、再建一次表纯属白干。
...
- **列表还没读回来就绝不写盘**（`editListRead`）。此时屏幕上的 `edits` 是空的，而 `SaveEdits` 交出的列表会被后端**整体替换**——那会把用户其余编辑一次抹掉。
...
这道门用 `FileStamp` 的**"确认生效"那一款**（不是"认领"那一款）：`Pending(current)` 在失败后仍然为真，"这一版处理完了"只能由调用方在确实生效之后用 `MarkApplied` 宣告。于是不需要"重试"这件事单独存在——失败自然会让下一拍再来。
...
- **拒写不推进版本**，这就是重试的全部机制。附带两条：
  ...
  - **同一版本只报一次**：`_quiet = (stamp == _loggedAttemptUtc)` 让 `LogAttempt` 在重试时静默，版本一变就重新开口。"这条表读不出来"与"原生拒写"在屏幕上都是同一件事，重复说没有新信息。原生侧那条 `WriteSkillStatusTable: refused (码)` 同理由它自己的 `last_refusal` 只报一次（拒绝码变了或中间成功过一次才再报）。
