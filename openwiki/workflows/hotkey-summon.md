...
游戏里按一下 F1（可配置），`SigilLoadout.exe` 的窗口就显出来；再按一下，它收回去。这条链横跨两个进程：注册热键和执行 `SetForegroundWindow` 的是**游戏进程里的托管 mod**，真正决定"显还是收"的是**工具进程自己**。两端之间只有窗口消息，**不传任何数据**——`0x8010` 与 `0x8012` 都是零参数命令。
...
`0x8011` 不在对拍名单里，因为它只有一处声明——工具内部 post 给自己，没有第二方要跟它对齐。
...
窗口三态与三条入口：只有热键带开关语义，X 按钮只会收，托盘与第二实例只会显；唯一能把窗口真正销毁的是置了 `quitting` 的 `WM_CLOSE`。

- **热键 `0x8012`**：先记下"工具抢走焦点之前的前台窗口"，再按 `toolHidden` 决定 `revealTool` + `Restore`/`Show`/`Focus`，还是 `fakeHide`。
- **X 按钮 `WM_CLOSE`**：`quitting` 未置位时只假隐藏（WebView 保持活着）；置位时（框架 `cleanup()` → `window.Close()`）交回默认处理，窗口真的销毁。`quitting` 必须在 `window.Close()` 之前立起来，否则退出会被改写成假隐藏。
- **托盘左键与第二实例 `0x8010`**：永远是"显出来"。托盘只在"窗口不是前台"或"正假隐藏"时才 post，否则每个左键点击都会重放一次激活；第二实例走的是 `startup.go` 里那条 `ShowWindow` + post + `SetForegroundWindow` + `os.Exit(0)`。

（工具自己还有第四条**内部**路径 `0x8011`：前端 Esc → `MinimiseApp` → `hideToTray`。它不与 mod 通信，所以不在本页的消息表里。）
...
这是一道对拍门，**不是"边界已证明"**：常量还要求两侧改不改、为什么是这个值，本页这张表与源码注释两边都得维护。
