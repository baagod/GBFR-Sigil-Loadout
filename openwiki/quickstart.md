...
| **改界面外壳（React 侧）**（页签、语言切换、焦点与 Esc、i18n 文案、落盘顺序与失败提示） | [可视工具前端（React）](/openwiki/architecture/visual-tool-frontend.md) → [工作流：配装从界面到游戏状态](/openwiki/workflows/loadout-apply.md)（编辑通路与载荷规则；注意「切换语言」也走同一条写盘通路） → [可视工具（Go + Wails）](/openwiki/architecture/visual-tool.md)（前端那些调用落到的 Go 侧） | `SigilLoadout/frontend/src/App.tsx`，以及同级的 `lang.ts`、`messages.ts`；入口与兜底在 `main.tsx`、`ErrorBoundary.tsx`，纯逻辑规则在 `model.ts`、`skills.ts`，面板在 `ExclusivePanel.tsx`、`SigilEditorPanel.tsx`、`SkillPicker.tsx` |
| **改数据资产与语言表**（`assets\` 下九份） | [外部生成器 gen 与随包数据资产](/openwiki/integrations/external-generator-and-assets.md) → [验证地图](/openwiki/testing/verification-map.md)（资产不变量由 Go 与前端测试守） | `SigilLoadout\assets\`；门禁在 `tools\build-release.ps1`（`sigils.json` 存在性与新鲜度、`sigils.chara.json` 的入库对账、必需文件清单） |
