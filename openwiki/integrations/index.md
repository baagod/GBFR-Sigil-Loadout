...
- [外部生成器 gen 与随包数据资产](external-generator-and-assets.md) - 仓库外的共享生成器 gen 与 SigilLoadout\assets\ 九份随包 JSON 的关系：谁产出哪一份、谁消费、哪些入库哪些是构建中间产物（exclusive_table.inc 与 sigils.chara.json 的差别）、vcxproj 的 GenerateExclusiveTable 目标与 build-release.ps1 的资产补齐与必需清单门禁，以及「本仓库无法单独完成一次发布构建」这条硬前提。
