/*
  语言身份：这个工具支持哪几种、每种在切换键上叫什么、以及"系统要哪一种"。
...
/*
  整个工具只有这一处"系统要什么语言"的猜测——只在配置里还没写语言时用一次（语言本身存在
  loadout.json 的 lang 里，由 App 读写）。

  以前这里还往 localStorage 存语言（当时 SigilEdit 是独立工具）。合并进
  PreEquippedSigils 后那第二个开关和第二条存储路径都没有理由留下：同一个窗口里两套语言
  状态，用户在一页切的语言不会带到另一页。
*/
