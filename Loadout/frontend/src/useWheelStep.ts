...
/**
 * 用滚轮步进：React 把 wheel 监听器注册为 passive，所以写在 onWheel prop 里的
 * preventDefault 毫无作用——浏览器照旧滚动，而数值同时也在变。监听器因此必须是原生的，
 * 并以 passive: false 添加。
 *
 * 一次注册要读到渲染中途才有的最新数值，所以两个回调放在 latest-props ref 里，
 * 而不是写进依赖数组——它们变了不该换一个监听器。
 *
