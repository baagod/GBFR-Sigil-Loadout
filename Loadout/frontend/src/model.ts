...
/** One row of sigils.chara.json (generated by gen's `exclusive` command):
 ...
 * loadout.json 的状态键是**技能** hash（`[1]`），而名字表 sigils.lang.json 以
...
/** The trust boundary for sigils.chara.json: 形状不完整的记录整条丢掉。非数组直接抛，
 ...
 * exclusive 全空时不写这个成员。抽成纯函数是为了能对真实 sigils.json 测它。
 */
