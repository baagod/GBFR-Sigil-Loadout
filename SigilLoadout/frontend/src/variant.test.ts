/*
    主因子下拉的值是**组键**（同一技能的变体共享），而 loadout.json 里记的是**物品** hash。
    载入时把前者翻成后者，若不留住后者，一个组里"名字不同的两个变体"就分不出来了：钳蟹的
    共鸣（1C4D37E4，无固定副技能）与永恒钳蟹因子（426AD20E，固定副技能 D3B8C21F）共享技能
    082033CB，是当前表里唯一这样的一族。这里钉住 载入 → 保存 的往返。
*/
...
describe("载入 → 保存的变体往返", () => {
    it("保留存档里指名的那个变体，不改写成组里第一行", () => {
        ...
        expect(slots[0].mainGem).toBe("426AD20E");
        // 保存：没被碰过的那一行必须照原样写回。
