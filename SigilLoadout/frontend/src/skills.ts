...
/*
  ...
  两条共享同一地址时，游戏最终拿到的是最后一条已启用的（PatchRows，SigilEditorFeature.cs）。文件里
  仍可能同时留着两条——旧版本可视工具写的，或者有人手改了 sigiledits.json——而列表只能显示其中
...
/*
  ...
  挡住科学计数法是关键：数字输入框以前会接受 1e999，JSON 把那个值变成 null，可视工具再把它
  存成 0。
