...
/*
  ...
  原因是行会在指针底下移动：一次勾选会把打开的内容排到最前，于是行动了、指针没动；
  而在同一行内移动不会触发 enter。hover 事件说不出此刻指针下是哪一行，所以这里用
  elementFromPoint 问文档，再把答案交给各行自己去比。

  base-ui 的 tooltip 还有一层脾气：它只在打开它的那次事件是 mouseenter/mousemove 时才
  让弹层跟着光标（useClientPoint 检查的正是这一点），而一次点击之后它根本不允许 hover
  打开，直到指针离开这一行再回来。所以"指针进入"与"勾选之后重新指向"都要把进入过程
  在行上重放一遍（见 replayHover）。

  这份状态住在列表那一层，因为"哪一行被指向"是整份列表的答案，而行自己只需要拿它跟
  自己的 id 比一比。
*/
export function useRowTooltip<T extends HTMLElement>() {
  const [hoveredRow, setHoveredRow] = useState<string | null>(null)
  // 指针最后一次移动的位置。勾选会在指针没动的情况下移动行，所以重解"指针下是哪一行"
  // 只能靠记下来的坐标。
  ...
  /*
    ...
    那个勾选框走：它把该行滚回视野，这正是让勾选感觉列表跳了一下的原因。偏移在下面的
    layout effect 里放回去，和把 tooltip 重新指向此刻指针下那一行是同一次 pass，
    于是两者都落在指针所在的位置。
  */
  ...
  /*
    把"进入这一行"在行上重放一遍。moveOnly 为 true 时只补一个 move —— 那种情形弹层
    已经在行上打开了（见 resolveRowUnderPointer），先来一个 leave 只会又把它关上。
  */
  ...
  /*
    指针此刻在哪一行，是问文档而不是问 hover 事件（原因见文件头）。调用点：勾选之后的
    layout effect，以及滚动关掉说明之后指针的第一次移动。
  */
  ...
  /*
    指针在列表里移动：记下位置；说明已被滚动关掉时替指针重解一次——那种情形指针常停在
    原行上，不会再触发 enter。
  */
  ...
  /*
    指针进入一行：记下位置并重放整个进入过程（原因见 replayHover）。能改变 hoveredRow 的
    只有指针从一行移到另一行，或者一次勾选——那是行在动，由上面那个 layout effect 处理。
  */
