/*
    ...
    与文案分开，因为两者变化的原因不同：加一种语言要动这里和 messages.ts 各一次，而改一句话只动
    messages.ts。
*/
...
/*
    每种语言用它自己的使用者第一眼认得的形式——中 / EN / 日 / 한——而不是四个语言代码：语言切换
    只有在看的人能找到自己那一项时才有用。用文字而不是国旗 emoji：Windows 不带国旗字形，
    画出来就是两个字母。
*/
...
/*
    ...
    语言只存 loadout.json 一处，不进 localStorage：同一个窗口里两套语言状态的话，用户在一页切的
    语言不会带到另一页。
*/
