...
// TestVirtualSlotCapacityFitsPlayerSlots：容量不够不会编译失败，只会让 ApplyLoadout 截断多出来的槽
// （只有日志会说）。三处 MaxSlots 的相等由上面那次对拍负责，所以这里只看原生容不容得下。
func TestVirtualSlotCapacityFitsPlayerSlots(t *testing.T) {
	const native = "../GBFR.SigilLoadout.Native/native_internal.h"
	source, err := os.ReadFile(filepath.FromSlash(native))
	if err != nil {
		t.Fatalf("reading %s: %v", native, err)
	}
	number := func(pattern string) int {
		t.Helper()
		match := regexp.MustCompile(pattern).FindStringSubmatch(string(source))
		if match == nil {
			t.Fatalf("%s 里找不到 %s", native, pattern)
		}
