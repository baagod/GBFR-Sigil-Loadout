...
# 产物 dist\GBFR-Pre-Equipped-Sigils-<version>.zip；结束会自动启动可视工具（Loadout.exe）
...
- **可视工具的编译检查用 `go vet ./...`（或 `go build -o <临时路径>`），不要裸跑 `go build`**：模块名是 `loadouttool`，
...
- 部署即 `deploy.ps1` 的行为：停 Loadout.exe → 覆盖 Mods 目标（默认
  `C:\Users\baago\Desktop\Reloaded-II\Mods\GBFR.PreEquippedSigils`，`-Target` 可覆盖）→ 重开可视工具。
  **游戏在运行会直接报错**（其 DLL 被加载中）。

## 发布（版本号同步）

1. 改 `ModConfig.json` 的 `ModVersion`（**唯一权威源**），以及可视工具前端的 `Loadout\frontend\package.json`
...
- **改专属数据（技能 / 因子 / 等级）**：改 `docs\tool-gen-loadout.ps1` 的 `$chars` 表 → 编译
  （vcxproj 编译前自动重跑生成器，重生成 `src\exclusive_table.inc` 与 `Loadout\assets\gem.chara.json`）
  → 部署 → 验证。
- **改通用槽 / 前端规则**：可视工具与托管逻辑（无内置通用默认；副技能规则见 `MAINTENANCE.md` §5）。
