...
bool TryLoadVirtualSkillSelection(
   ...
   try {
      ...
      Log("virtual skill selection: threw; treated as no selection.");
      selection = {};
      return false;
   }
}
...
void DisableGameplayHooksAndRestore() noexcept {
   ...
   // safetyhook 的 stub 在它返回之后还要跑收尾指令——计数器先归零，reset() 就会 VirtualFree 掉线程
   // 仍在执行的那几页。disable() 已还原目标字节，留几页到进程退出是安全的。
