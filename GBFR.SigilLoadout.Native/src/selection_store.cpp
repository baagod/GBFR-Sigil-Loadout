...
void RebuildPartyStatusesOnce()
{
   ...
   //
   // 为什么必须有这条路：**战斗里游戏自己不会重建角色状态**（2026-09-21 实测：带满队友进
   // 真实副本、整场不改配置，4 分 09 秒里零次构建；对照进副本前在城里那 90 秒，玩家自己被
   // 重建了 6 次）。所以"等下一场战斗"在战斗中永远等不到——发布配装时这次主动重建，是唯一
   // 能让改动在同一场战斗内生效的路。它也是本项目唯一会去动游戏活对象的地方。
